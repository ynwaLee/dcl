...
)
{
   ...
   for (n = 0; n < L; n++)
   {
     s = 0;
     for (i = 0; i <= n; i++)
       s = L_mac(pgd, s, x[i], h[n-i]);

     s    = L_shl(pgd, s, 3);                   /* h is in Q12 and saturation */
...
)
{
  ...
  for (i = 0; i < lg; i++)
  {
    s = L_mult(pgd, x[i], a[0]);
    for (j = 1; j <= M; j++)
      s = L_msu(pgd, s, a[j], yy[-j]);
...
)
{
  ...
  for (i = 0; i < lg; i++)
  {
    s = L_mult(pgd, x[i], a[0]);
    for (j = 1; j <= M; j++)
      s = L_mac(pgd, s, a[j], x[i-j]);

    s = L_shl(pgd, s, 3);
    y[i] = g729_round(pgd,s);
  }
  return;
}
