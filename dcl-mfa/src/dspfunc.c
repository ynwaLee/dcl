...
/*___________________________________________________________________________
 ...
 |   4- L_x = L_x >> (30-exponent)     (with g729_rounding)                       |
 |___________________________________________________________________________|
*/
...
)
{
  ...
  L_x = L_mult(pgd, fraction, 32);           /* L_x = fraction<<6           */
  i   = g729_extract_h(L_x);                 /* Extract b10-b15 of fraction */
  L_x = L_shr(pgd, L_x, 1);
  a   = g729_extract_l(L_x);                 /* Extract b0-b9   of fraction */
  a   = a & (Word16)0x7fff;

  L_x = g729_L_deposit_h(tabpow[i]);          /* tabpow[i] << 16        */
  tmp = sub(pgd,tabpow[i], tabpow[i+1]);      /* tabpow[i] - tabpow[i+1] */
  L_x = L_msu(pgd, L_x, tmp, a);             /* L_x -= tmp*a*2        */

  exp = sub(pgd,30, exponent);
  L_x = L_shr_r(pgd,L_x, exp);
...
)
{
  ...
  exp = g729_norm_l(L_x);
  L_x = L_shl(pgd, L_x, exp );               /* L_x is normalized */

  *exponent = sub(pgd,30, exp);
  ...
  L_y = g729_L_deposit_h(tablog[i]);          /* tablog[i] << 16        */
  tmp = sub(pgd,tablog[i], tablog[i+1]);      /* tablog[i] - tablog[i+1] */
  L_y = L_msu(pgd, L_y, tmp, a);             /* L_y -= tmp*a*2        */
...
)
{
  ...
  exp = g729_norm_l(L_x);
  L_x = L_shl(pgd, L_x, exp );               /* L_x is normalize */
  ...
  L_y = g729_L_deposit_h(tabsqr[i]);          /* tabsqr[i] << 16          */
  tmp = sub(pgd,tabsqr[i], tabsqr[i+1]);      /* tabsqr[i] - tabsqr[i+1])  */
  L_y = L_msu(pgd, L_y, tmp, a);             /* L_y -=  tmp*a*2         */

  L_y = L_shr(pgd, L_y, exp);                /* denormalization */
