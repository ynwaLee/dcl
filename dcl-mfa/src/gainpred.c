...
)
{
   ...
   *-------------------------------*/
   ...
   L_tmp = 0;
   for(i=0; i<L_subfr; i++)
     L_tmp = L_mac(pgd, L_tmp, code[i], code[i]);
   ...
   *-----------------------------------------------------------------*/
   ...
   Log2(pgd, L_tmp, &exp, &frac);               /* Q27->Q0 ^Q0 ^Q15       */
   L_tmp = Mpy_32_16(pgd, exp, frac, -24660);   /* Q0 Q15 Q13 -> ^Q14     */
                                           /* hi:Q0+Q13+1            */
                                           /* lo:Q15+Q13-15+1        */
                                           /* -24660[Q13]=-3.0103    */
   L_tmp = L_mac(pgd, L_tmp, 32588, 32);        /* 32588*32[Q14]=127.298  */
   ...
   *-----------------------------------------------------------------*/
   ...
   L_tmp = L_shl(pgd,L_tmp, 10);                      /* From Q14 to Q24 */
   for(i=0; i<4; i++)
     L_tmp = L_mac(pgd, L_tmp, pred[i], past_qua_en[i]); /* Q13*Q10 ->Q24 */
   ...
   *-----------------------------------------------------------------*/
   ...
   L_tmp = L_mult(pgd, *gcode0, 5439);       /* *0.166 in Q15, result in Q24*/
   L_tmp = L_shr(pgd,L_tmp, 8);             /* From Q24 to Q16             */
   L_Extract(pgd, L_tmp, &exp, &frac);       /* Extract exponent of gcode0  */

   *gcode0 = g729_extract_l(Pow2(pgd, 14, frac)); /* Put 14 as exponent so that  */
                                        /* output of Pow2() will be:   */
                                        /* 16768 < Pow2() <= 32767     */
   *exp_gcode0 = sub(pgd,14,exp);
}
...
)
{
   ...
   *----------------------------------------------------------------------*/
   ...
   Log2(pgd, L_gbk12, &exp, &frac );               /* L_gbk12:Q13       */
   L_acc = L_Comp(pgd, sub(pgd,exp,13), frac);         /* L_acc:Q16           */
   tmp = g729_extract_h( L_shl(pgd, L_acc,13 ) );       /* tmp:Q13           */
   past_qua_en[0] = mult(pgd, tmp, 24660 );        /* past_qua_en[]:Q10 */
}
...
)
{
   ...
   L_tmp = 0;                                                    /* Q10 */
   for(i=0; i<4; i++)
      L_tmp = L_add(pgd, L_tmp, g729_L_deposit_l( past_qua_en[i] ) );
   av_pred_en = g729_extract_l( L_shr(pgd, L_tmp, 2 ) );
   av_pred_en = sub(pgd, av_pred_en, 4096 );                          /* Q10 */

   if( sub(pgd,av_pred_en, -14336) < 0 ){
