/* ITU-T G.729 Software Package Release 2 (November 2006) */
/* ITU-T G.729 - Reference C code for fixed point implementation */
/* Version 3.3    Last modified: December 26, 1995 */

#include <stdio.h>
#include "typedef.h"
#include "basic_op.h"
#include "ld8k.h"


void Lsp_get_quant(G729DECODER * pgd,
  Word16 lspcb1[][M],      /* (i) Q13 : first stage LSP codebook      */
  Word16 lspcb2[][M],      /* (i) Q13 : Second stage LSP codebook     */
  Word16 code0,               /* (i)     : selected code of first stage  */
  Word16 code1,               /* (i)     : selected code of second stage */
  Word16 code2,               /* (i)     : selected code of second stage */
  Word16 fg[][M],          /* (i) Q15 : MA prediction coef.           */
  Word16 freq_prev[][M],   /* (i) Q13 : previous LSP vector           */
  Word16 lspq[],              /* (o) Q13 : quantized LSP parameters      */
  Word16 fg_sum[]             /* (i) Q15 : present MA prediction coef.   */
)
{
  Word16 j;
  Word16 buf[M];           /* Q13 */


  for ( j = 0 ; j < NC ; j++ )
    buf[j] = add(pgd, lspcb1[code0][j], lspcb2[code1][j] );

  for ( j = NC ; j < M ; j++ )
    buf[j] = add(pgd, lspcb1[code0][j], lspcb2[code2][j] );

  Lsp_expand_1_2(pgd, buf, GAP1);
  Lsp_expand_1_2(pgd, buf, GAP2);

  Lsp_prev_compose(pgd, buf, lspq, fg, freq_prev, fg_sum);

  Lsp_prev_update(buf, freq_prev);

  Lsp_stability(pgd, lspq );

  return;
}


void Lsp_expand_1(G729DECODER * pgd,
  Word16 buf[],        /* (i/o) Q13 : LSP vectors */
  Word16 gap           /* (i)   Q13 : gap         */
)
{
  Word16 j, tmp;
  Word16 diff;        /* Q13 */

  for ( j = 1 ; j < NC ; j++ ) {
    diff = sub(pgd, buf[j-1], buf[j] );
    tmp = shr(pgd, add(pgd, diff, gap), 1 );

    if ( tmp >  0 ) {
      buf[j-1] = sub(pgd, buf[j-1], tmp );
      buf[j]   = add(pgd, buf[j], tmp );
    }
  }
  return;
}


void Lsp_expand_2(G729DECODER * pgd,
  Word16 buf[],       /* (i/o) Q13 : LSP vectors */
  Word16 gap          /* (i)   Q13 : gap         */
)
{
  Word16 j, tmp;
  Word16 diff;        /* Q13 */

  for ( j = NC ; j < M ; j++ ) {
    diff = sub(pgd, buf[j-1], buf[j] );
    tmp = shr(pgd, add(pgd, diff, gap), 1 );

    if ( tmp > 0 ) {
      buf[j-1] = sub(pgd, buf[j-1], tmp );
      buf[j]   = add(pgd, buf[j], tmp );
    }
  }
  return;
}


void Lsp_expand_1_2(G729DECODER * pgd,
  Word16 buf[],       /* (i/o) Q13 : LSP vectors */
  Word16 gap          /* (i)   Q13 : gap         */
)
{
  Word16 j, tmp;
  Word16 diff;        /* Q13 */

  for ( j = 1 ; j < M ; j++ ) {
    diff = sub(pgd, buf[j-1], buf[j] );
    tmp = shr(pgd, add(pgd, diff, gap), 1 );

    if ( tmp > 0 ) {
      buf[j-1] = sub(pgd, buf[j-1], tmp );
      buf[j]   = add(pgd, buf[j], tmp );
    }
  }
  return;
}


/*
  Functions which use previous LSP parameter (freq_prev).
*/


/*
  compose LSP parameter from elementary LSP with previous LSP.
*/
void Lsp_prev_compose(G729DECODER * pgd,
  Word16 lsp_ele[],             /* (i) Q13 : LSP vectors                 */
  Word16 lsp[],                 /* (o) Q13 : quantized LSP parameters    */
  Word16 fg[][M],            /* (i) Q15 : MA prediction coef.         */
  Word16 freq_prev[][M],     /* (i) Q13 : previous LSP vector         */
  Word16 fg_sum[]               /* (i) Q15 : present MA prediction coef. */
)
{
  Word16 j, k;
  Word32 L_acc;                 /* Q29 */

  for ( j = 0 ; j < M ; j++ ) {
    L_acc = L_mult(pgd, lsp_ele[j], fg_sum[j] );
    for ( k = 0 ; k < MA_NP ; k++ )
      L_acc = L_mac(pgd, L_acc, freq_prev[k][j], fg[k][j] );

    lsp[j] = g729_extract_h(L_acc);
  }
  return;
}


/*
  extract elementary LSP from composed LSP with previous LSP
*/
void Lsp_prev_extract(G729DECODER * pgd,
  Word16 lsp[M],                 /* (i) Q13 : unquantized LSP parameters  */
  Word16 lsp_ele[M],             /* (o) Q13 : target vector               */
  Word16 fg[MA_NP][M],           /* (i) Q15 : MA prediction coef.         */
  Word16 freq_prev[MA_NP][M],    /* (i) Q13 : previous LSP vector         */
  Word16 fg_sum_inv[M]           /* (i) Q12 : inverse previous LSP vector */
)
{
  Word16 j, k;
  Word32 L_temp;                    /* Q19 */
  Word16 temp;                      /* Q13 */


  for ( j = 0 ; j < M ; j++ ) {
    L_temp = g729_L_deposit_h(lsp[j]);
    for ( k = 0 ; k < MA_NP ; k++ )
      L_temp = L_msu(pgd, L_temp, freq_prev[k][j], fg[k][j] );

    temp = g729_extract_h(L_temp);
    L_temp = L_mult(pgd, temp, fg_sum_inv[j] );
    lsp_ele[j] = g729_extract_h( L_shl(pgd, L_temp, 3 ) );

  }
  return;
}


/*
  update previous LSP parameter
*/
void Lsp_prev_update(
  Word16 lsp_ele[M],             /* (i)   Q13 : LSP vectors           */
  Word16 freq_prev[MA_NP][M]     /* (i/o) Q13 : previous LSP vectors  */
)
{
  Word16 k;

  for ( k = MA_NP-1 ; k > 0 ; k-- )
    g729_Copy(freq_prev[k-1], freq_prev[k], M);

  g729_Copy(lsp_ele, freq_prev[0], M);
  return;
}

void Lsp_stability(G729DECODER * pgd,
  Word16 buf[]       /* (i/o) Q13 : quantized LSP parameters      */
)
{
  Word16 j;
  Word16 tmp;
  Word32 L_diff;
  Word32 L_acc, L_accb;

  for(j=0; j<M-1; j++) {
    L_acc = g729_L_deposit_l( buf[j+1] );
    L_accb = g729_L_deposit_l( buf[j] );
    L_diff = L_sub(pgd, L_acc, L_accb );

    if( L_diff < 0L ) {
      /* exchange buf[j]<->buf[j+1] */
      tmp      = buf[j+1];
      buf[j+1] = buf[j];
      buf[j]   = tmp;
    }
  }


  if( sub(pgd,buf[0], L_LIMIT) <0 ) {
    buf[0] = L_LIMIT;
    printf("lsp_stability warning Low \n");
  }
  for(j=0; j<M-1; j++) {
    L_acc = g729_L_deposit_l( buf[j+1] );
    L_accb = g729_L_deposit_l( buf[j] );
    L_diff = L_sub(pgd, L_acc, L_accb );

    if( L_sub(pgd,L_diff, GAP3)<0L ) {
      buf[j+1] = add(pgd, buf[j], GAP3 );
    }
  }

  if( sub(pgd,buf[M-1],M_LIMIT)>0 ) {
    buf[M-1] = M_LIMIT;
    printf("lsp_stability warning High \n");
  }
  return;
}
