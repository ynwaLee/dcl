...
static int ma_cmd_reload(uint8_t *data, uint32_t data_len)
{
    ...
    for (i = 0; i < num; i++) {
        ...
        else if (strcmp(conf[i], MFU_CONF_FILE) == 0) {
            flag |= CONF_RELOAD_MFU;
        }
        else if (strcmp(conf[i], "all") == 0) {
            flag = CONF_RELOAD_ALL;
        }else {
            xapplog(LOG_WARNING, APP_LOG_MASK_MA, "invalied filename when reloading!");
            return 0;
        }
    }
...
static int ma_cmd_send_counter(int sockfd)
{
    ...
    uint16_t pkt_len;
    int mfu_counter_cmd = 20;

    mfu_counter_t counter = {0};
    int counterlen = sizeof(mfu_counter_t);
    bcopy(&g_mfu_counter, &counter, counterlen);

    xapplog(LOG_INFO, APP_LOG_MASK_BASE, "media [request (total: %llu, valid: %llu, invalid: %llu) response(total: %llu, valid: %llu, invalid: %llu)], svmtelphone [request (total: %llu, valid: %llu, invalid: %llu), response(total: %llu, valid: %llu, invalid: %llu)], svmmobile [request (total: %llu, valid: %llu, invalid: %llu), response(total: %llu, valid: %llu, invalid: %llu)]", 
    counter.media_req_total, 
    counter.media_req_valid, 
    counter.media_req_invalid, 
    counter.media_res_total, 
    counter.media_res_valid, 
    counter.media_res_invalid,

    counter.svmtel_req_total, 
    counter.svmtel_req_valid, 
    counter.svmtel_req_invalid, 
    counter.svmtel_res_total, 
    counter.svmtel_res_valid, 
    counter.svmtel_res_invalid,

    counter.svmmob_req_total, 
    counter.svmmob_req_valid, 
    counter.svmmob_req_invalid, 
    counter.svmmob_res_total, 
    counter.svmmob_res_valid, 
    counter.svmmob_res_invalid);

    counter.media_req_total = htobe64(counter.media_req_total);
    counter.media_req_valid = htobe64(counter.media_req_valid);
    counter.media_req_invalid = htobe64(counter.media_req_invalid);
    counter.media_res_total = htobe64(counter.media_res_total);
    counter.media_res_valid = htobe64(counter.media_res_valid);
    counter.media_res_invalid = htobe64(counter.media_res_invalid);

    counter.svmtel_req_total = htobe64(counter.svmtel_req_total);
    counter.svmtel_req_valid = htobe64(counter.svmtel_req_valid);
    counter.svmtel_req_invalid = htobe64(counter.svmtel_req_invalid);
    counter.svmtel_res_total = htobe64(counter.svmtel_res_total);
    counter.svmtel_res_valid = htobe64(counter.svmtel_res_valid);
    counter.svmtel_res_invalid = htobe64(counter.svmtel_res_invalid);

    counter.svmmob_req_total = htobe64(counter.svmmob_req_total);
    counter.svmmob_req_valid = htobe64(counter.svmmob_req_valid);
    counter.svmmob_req_invalid = htobe64(counter.svmmob_req_invalid);
    counter.svmmob_res_total = htobe64(counter.svmmob_res_total);
    counter.svmmob_res_valid = htobe64(counter.svmmob_res_valid);
    counter.svmmob_res_invalid = htobe64(counter.svmmob_res_invalid);

    pkt_len = fill_ma_counter2(pkt_buf, mfu_counter_cmd, g_counter_sn, (uint8_t*)&counter, counterlen);
    
    ret = sock_send(sockfd, pkt_buf, pkt_len, &sended);
    if (ret == -1) {
        xapplog(LOG_ERR, APP_LOG_MASK_MA, "send counter data less than pkt_len %dbytes", pkt_len);
        return -1;
    }
    return 0;
}
...
static int ma_cmd_parser(uint16_t cmd, uint16_t sn, uint8_t *pkt, uint16_t pkt_len)
{
    ...
    switch(cmd) {
        case COUNTER_STRUCT:
            xapplog(LOG_INFO, APP_LOG_MASK_MA, "get counter struct cmd from ma");
            ret = fill_ma_count_info_struct(pkt_buf, sn, "mfu", "mfu", g_mfu_count_struct_num, g_mfu_count_struct);
...
static int macli_para_init(macli_para_t *maclipara, cmdline_para_t *cmdlpara)
{
    ...
    if (ret == 0) {
        ...
        applog(LOG_INFO, APP_LOG_MASK_CDR, "mfu's sn is not set or error, won't connect to ma.");
    }
