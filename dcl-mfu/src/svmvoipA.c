...
static int valcopy_voipA(unsigned long srcval, int size, void *dstbuf, int offset, int flag)
{
    if (flag == 0) {                                     
...
static int pack_svmvoipA_req(char *buf, http_reqpara_t *reqpara) 
{
    ...
    offset = 0;
    //type len
    offset = valcopy_voipA(0x10, 1, buf, offset, 0);
    offset = valcopy_voipA(0x00, 2, buf, offset, 0);
#if 1
    //systype  
    offset = valcopy_voipA(0x02, 2, buf, offset, 0);
    offset = valcopy_voipA(0x04, 2, buf, offset, 0);
    offset = valcopy_voipA(reqpara->systype, 4, buf, offset, 0);
#endif

    //type len online
    offset = valcopy_voipA(0x00, 2, buf, offset, 0);
    offset = valcopy_voipA(0x04, 2, buf, offset, 0);
    offset = valcopy_voipA(reqpara->online, 4, buf, offset, 0);

    //type len mntid
    offset = valcopy_voipA(0x01, 2, buf, offset, 0);
    offset = valcopy_voipA(0x08, 2, buf, offset, 0);
    offset = valcopy_voipA(reqpara->clueid, 8, buf, offset, 0);

    //type len channel
    offset = valcopy_voipA(0x02, 2, buf, offset, 0);
    offset = valcopy_voipA(0x04, 2, buf, offset, 0);
    offset = valcopy_voipA(reqpara->callcode, 4, buf, offset, 0);

    //type len vocfile
    unsigned int filesize = strlen(reqpara->vocfile) + 1;
    offset = valcopy_voipA(0x03, 2, buf, offset, 0);
    offset = valcopy_voipA(filesize, 2, buf, offset, 0);
    offset = valcopy_voipA((unsigned long)reqpara->vocfile, filesize, buf, offset, 1);

    unsigned int len = offset - 3;
    valcopy_voipA(len, 2, buf, 1, 0);
...
static int get_valid_svmvoipAfd(ipport_t *svmvoipa_addr, http_reqpara_t *reqpara, unsigned int *filesize)
{
    int sended, recved;
    char *reqbuf = alloca(MAX_PARA_VALUE_LEN + 16); 
    int len = pack_svmvoipA_req(reqbuf, reqpara);

    int tmpfd;
    ipport_t *addr = svmvoipa_addr;
	//是否有城市码对应的ip
	
	if(strlen(reqpara->city_code) != 0)
	{
	    for (; addr != NULL; addr = addr->next) 
		{
		    if(!strcmp(addr->city_code , reqpara->city_code) && (strlen(addr->city_code)==strlen(reqpara->city_code)))
		    {
		        if ((tmpfd = sock_connect(addr->ip, addr->port)) == -1)
				{
                    xapplog(LOG_ERR, APP_LOG_MASK_BASE, "citycode = %s connect %s %d failed.", reqpara->city_code , addr->ip, addr->port);
                    return -1;
                }

                if (sock_send(tmpfd, reqbuf, len, &sended) == -1) 
				{
                    xapplog(LOG_ERR, APP_LOG_MASK_BASE, "citycode = %s sent request to %s %d failed.", reqpara->city_code ,addr->ip, addr->port);
                    close(tmpfd);
                    return -1;
                }
                ...
                if (sock_recv(tmpfd, resbuf, 12, &recved) <= 0) {
                    xapplog(LOG_ERR, APP_LOG_MASK_BASE, "citycode = %s recv response from %s %d failed.", reqpara->city_code , addr->ip, addr->port);
                    close(tmpfd);
                    return -1;
                }
...
static int transfer_data_stream(int svmvoipafd, int sockfd, unsigned int filesize)
{
    int sended;
    char sendbuf[4096] = {0};
    int len;
    ...
    while (1)
    {
        ...
        struct timeval tv;
        int ret;
        int maxfd = (svmvoipafd > sockfd) ? svmvoipafd : sockfd;

        tv.tv_sec = 10;
        tv.tv_usec = 0;
        FD_ZERO(&rfds);
        FD_SET(svmvoipafd, &rfds);
        FD_SET(sockfd, &rfds);
        ...
        if (ret > 0 && FD_ISSET(svmvoipafd, &rfds)) {
            len = recv(svmvoipafd, sendbuf, 512, 0);
            if (len == -1) {
...
void *svmvoipA_thread(void *arg)
{
    int ret;
    int sockfd = (int)(unsigned long)arg;
    mfusrv_para_t *conf = mfusrv_para_get();
    ipport_t *svmvoipA_addr = conf->svmvoipA_addr;
    
    atomic_add(&g_mfu_counter.svmvoipA_req_total, 1);
    char buf[MAX_HTTP_HEADER_LEN] = {0};
    ret = recv(sockfd, buf, MAX_HTTP_HEADER_LEN - 1, 0);
    if (ret <= 0) {
        atomic_add(&g_mfu_counter.svmvoipA_req_invalid, 1);
        xapplog(LOG_ERR, APP_LOG_MASK_BASE, "svmvoipA recv media request failed.");
        close(sockfd);
        return NULL;
    }
    ...
    atomic_add(&g_mfu_counter.svmvoipA_req_valid, 1);
    
    atomic_add(&g_mfu_counter.svmvoipA_res_total, 1);
    int svmvoipAfd = 0;
    unsigned int filesize = 0;
    if ((svmvoipAfd = get_valid_svmvoipAfd(svmvoipA_addr, &reqpara, &filesize)) == -1) {
        atomic_add(&g_mfu_counter.svmvoipA_res_invalid, 1);
        xapplog(LOG_ERR, APP_LOG_MASK_BASE, "failed to get valid svmvoipA fd.");
        close(sockfd);
        return NULL;
    }

    if ((ret = transfer_data_stream(svmvoipAfd, sockfd, filesize)) == -1) {
    ...
    close(svmvoipAfd);
    close(sockfd);
    atomic_add(&g_mfu_counter.svmvoipA_res_valid, 1);
