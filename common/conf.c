...
void ConfInit(void)
{
    ...
    if (unlikely(root == NULL)) { 
        applog(LOG_ERR, APP_LOG_MASK_BASE, "ERROR: Failed to allocate memory");
        exit (-1);
    }
}
...
static int ConfGetChildValue(ConfNode *base, char *name, char **vptr)
{
 ...
 */
static int PathIsRelative(const char *path) {
 ...
 */
static char *ConfLoadCompleteIncludePath(char *file)
{
...
ConfYamlParse(yaml_parser_t *parser, ConfNode *parent, int inseq)
{
    ...
    while (!done) {
        if (!yaml_parser_parse(parser, &event)) {
            applog(LOG_ERR, APP_LOG_MASK_BASE,
        ...
        if (event.type == YAML_DOCUMENT_START_EVENT) {
            ...
            if (ver == NULL) {
                applog(LOG_ERR, APP_LOG_MASK_BASE, "ERROR: Invalid configuration file.");
                applog(LOG_ERR, APP_LOG_MASK_BASE, "The configuration file must begin with the following two lines:");
                applog(LOG_ERR, APP_LOG_MASK_BASE, "%%YAML 1.1\n---");
                goto fail;
            }
            ...
            if (!(major == YAML_VERSION_MAJOR && minor == YAML_VERSION_MINOR)) {
                applog(LOG_ERR, APP_LOG_MASK_BASE, "ERROR: Invalid YAML version.  Must be 1.1");
                goto fail;
            }
        }
                        ...
                        if (node->name && strchr(node->name, '_')) {
                            if (!(parent->name &&
                                ...
                                if (mangle_errors < MANGLE_ERRORS_MAX) {
                                    ...
                                    mangle_errors++;
                                    if (mangle_errors >= MANGLE_ERRORS_MAX)
                                        applog(LOG_ERR, APP_LOG_MASK_BASE, "not showing more "
...
int ConfYamlLoadFile(const char *filename)
{
    ...
    if (yaml_parser_initialize(&parser) != 1) {
        applog(LOG_ERR, APP_LOG_MASK_BASE, "ERROR: Failed to initialize yaml '%s' parser.", filename);
        return -1;
    }

    infile = fopen(filename, "r");
    if (infile == NULL) {
        applog(LOG_ERR, APP_LOG_MASK_BASE, "ERROR: Failed fopen '%s' file %s", filename, strerror(errno));
...
typedef yaml_char_t * yct;

/* Emitter emit yaml_root node to character string for saving */
static int ConfYamlEmit(yaml_emitter_t *emitter, ConfNode *parent, int inseq)
{
    ConfNode *node = TAILQ_FIRST(&parent->head);
    yaml_event_t event;
    bzero(&event,sizeof(yaml_event_t));
    ...
    if(inseq)
    {
        ...
        if(strcmp(node->name,"0") == 0)
        {
            ...
            while(node != NULL)
            {
                if(node->is_seq)
                {
                    ConfYamlEmit(emitter,node,0);
                }
                else
                {
                    yaml_scalar_event_initialize(&event,NULL,NULL,(yct)node->val,-1,1,1,YAML_PLAIN_SCALAR_STYLE);
                    yaml_emitter_emit(emitter,&event);
                }
...
//            yaml_emitter_emit(emitter,&event);
        ...
        else
        {
            ...
            while(node != NULL)
            {
                if(node->val)
                {
                    yaml_scalar_event_initialize(&event,NULL,NULL,(yct)node->name,-1,1,1,YAML_PLAIN_SCALAR_STYLE);
                    yaml_emitter_emit(emitter,&event);
                    yaml_scalar_event_initialize(&event,NULL,NULL,(yct)node->val,-1,1,1,YAML_PLAIN_SCALAR_STYLE);
                    yaml_emitter_emit(emitter,&event);
                }
    ...
    else
    {
        ...
        while(node != NULL)
        {
            if(node->val)
            {
                ...
                //ConfYamlEmit(emitter,node,1);
                //the line above is added by sxh
            }
            else
            {
                yaml_scalar_event_initialize(&event,NULL,NULL,(yct)node->name,-1,1,1,YAML_PLAIN_SCALAR_STYLE);
...
#define YAML_FILE_BUFFER_SIZE 65536

/**
 * \restore configuration to the source yaml file and override it.
 * reload it will use new configuration.
 * the program allow change it, and will not lose.
 */
int ConfYamlUpdateFile(const char *conf_file)
{
    FILE *outfile;
    unsigned char buffer[YAML_FILE_BUFFER_SIZE];
    ...
    if(yaml_emitter_initialize(&emitter) != 1)
        return -1;

    bzero(buffer,sizeof(buffer));
    yaml_emitter_set_output_string(&emitter,buffer,YAML_FILE_BUFFER_SIZE,&written);
    ...
    ret = ConfYamlEmit(&emitter,root,0); //Yaml_Emitter_Emit write data to string.
    ...
    if((outfile = fopen(conf_file,"w")) == NULL) {
        applog(LOG_ERR, APP_LOG_MASK_BASE, "ERROR:%s, Failed fopen config file '%s'.", strerror(errno), conf_file);
        return -1;
    }

    fwrite(buffer,1,strlen((const char *)buffer),outfile);
    fclose(outfile);
