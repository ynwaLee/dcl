...
#define DEF_APP_LOG_LEVEL LOG_DEBUG
...
extern unsigned int g_app_log_debug;

extern void send_to_ma_log(int level, const char *fmt, ...);

#define open_applog openlog
#define applog(x, y, ...)  if (g_app_log_level >= x) \
                           {  \
                               syslog(x, __VA_ARGS__); \
                           }  \
                           if ((g_app_log_debug)&&((g_app_log_level >= x)||(g_app_debug_mask&y))) \
                           {  \
                               send_to_ma_log(x, __VA_ARGS__); \
                           }
