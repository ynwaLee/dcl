...
int check_mobile_dir(void)
{
    struct stat dir_stat;
    char cmd[1024];
    char path[1024];

    snprintf(path, 1023, "%s/%s", cfu_conf.local_ftp_dir, MOBILE_DIR);
...
int mobile_mass_parser(char *buf, mobile_mass_t *mass)
{
    char *p;
    char *q;
    int n;
    ...
    p = buf;
    n = 0;
    memset(mass, 0, sizeof(mobile_mass_t));
    while (1) {
        q = strchr(p, ',');
        if (q == NULL) {
            break;
...
int mobile_case_parser(char *buf, mobile_case_t *cases)
{
    char *p;
    char *q;
    int n;
    ...
    p = buf;
    n = 0;
    memset(cases, 0, sizeof(mobile_case_t));
    while (1) {
        q = strchr(p, ',');
        if (q == NULL) {
            break;
...
int mobile_mass_convert(char *path, char *filename)
{
    FILE *fp_src;
    FILE *fp_dst;
    char buffer[1024] = {0};
    ...
    while(fgets(buffer, 1024, fp_src) != NULL) {
        return_value = mobile_mass_parser(buffer, &mobile_mass);
        if (return_value == 30) {
            atomic64_add(&mobile_mass_call_num, 1);
            mobile2das_mass(&mobile_mass, &das_record);
            write_bcp(fp_dst, &das_record);
            atomic64_add(&das_mass_call_num, 1);
            cdr_counter += 1;
            if (mobmass_fifo_push(&mobile_mass, &g_mobmass_fifo) == -1) {
                applog(APP_LOG_LEVEL_ERR, APP_LOG_MASK_TELE, "push mobile mass cdr to fifo error.");
            }
        }
    }
    //applog(APP_LOG_LEVEL_INFO, APP_LOG_MASK_TELE, "mobile mass convert %d cdr", cdr_counter);
...
int mobile_case_convert(char *path, char *filename)
{
    FILE *fp_src;
    FILE *fp_dst;
    char buffer[1024] = {0};
    ...
    cdr_counter = 0;
    while(fgets(buffer, 1024, fp_src) != NULL) {
        return_value = mobile_case_parser(buffer, &mobile_case);
        if (return_value != 31) {
            continue;
        }

        atomic64_add(&mobile_case_call_num, 1);
        clue_id = atoi(mobile_case.mntid) - 200;
        ...
        mobile2das_case(&mobile_case, &das_record, object_id);
    ...
    //applog(APP_LOG_LEVEL_INFO, APP_LOG_MASK_TELE, "mobile case convert %d cdr", cdr_counter);
...
int settle_mobile_history_file(void)
{
    char path[1024];
    DIR *fp;
    struct dirent dir;
        ...
        if (check_filetype(dir.d_name, ".csv") == 0) {
            ...
            if (offset != NULL) {
                if (offset - dir.d_name == 12) {
                    //mass
                    mobile_mass_convert(path, dir.d_name);
                    atomic64_add(&mobile_mass_file_num, 1);
                } else if (offset - dir.d_name == 14) {
                    //case
                    mobile_case_convert(path, dir.d_name);
                    atomic64_add(&mobile_case_file_num, 1);
                }
            }
        }
    }
...
void *thread_mobile_cdr(void *arg)
{
    ...
    snprintf(mobile_dir, 1023, "%s/%s", cfu_conf.local_ftp_dir, MOBILE_DIR);
    ...
    while (1) {
        ...
        if (fp_dir == NULL) {
            fp_dir = opendir(mobile_dir);
            if (fp_dir == NULL) {
                applog(APP_LOG_LEVEL_INFO, APP_LOG_MASK_BASE, "open %s failed, %s", mobile_dir, strerror(errno));
                continue;
            }
        }

        if (readdir_r(fp_dir, &dir, &dirp) != 0) {
            applog(APP_LOG_LEVEL_INFO, APP_LOG_MASK_BASE, "read %s error, %s", mobile_dir, strerror(errno));
            closedir(fp_dir);
            fp_dir = NULL;
            continue;
        }
        ...
        if (check_filetype(dir.d_name, ".csv") == 0) {
            ...
            if (offset != NULL) {
                if (offset - dir.d_name == 12) {
                    ...
                    //mass
                    mobile_mass_convert(mobile_dir, dir.d_name);
                    atomic64_add(&mobile_mass_file_num, 1);
                    //gettimeofday(&tv2,&tz2);
                    //applog(LOG_INFO, APP_LOG_MASK_SIGNAL, "mobile mass cdr convert to bcp spent %lu usec.", 
                            //(tv2.tv_usec>tv1.tv_usec) ? (tv2.tv_usec-tv1.tv_usec):(1000000+tv2.tv_usec-tv1.tv_usec));
                } else if (offset - dir.d_name == 14) {
                    //gettimeofday(&tv1,&tz1);
                    //case
                    mobile_case_convert(mobile_dir, dir.d_name);
                    atomic64_add(&mobile_case_file_num, 1);
                    //gettimeofday(&tv2,&tz2);
                    //applog(LOG_INFO, APP_LOG_MASK_SIGNAL, "mobile case cdr convert to bcp spent %lu usec.", 
                            //(tv2.tv_usec>tv1.tv_usec) ? (tv2.tv_usec-tv1.tv_usec):(1000000+tv2.tv_usec-tv1.tv_usec));
                } else {
                    //error
                    applog(APP_LOG_LEVEL_DEBUG, APP_LOG_MASK_TELE, 
                            "mobile file error: %s", dir.d_name);
                }
            }
        }
